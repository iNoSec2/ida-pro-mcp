    ...
    for name, (config_dir, config_file) in configs.items():
        ...
        if uninstall:
            if mcp.name not in mcp_servers:
                if not quiet:
                    print(f"Skipping {name} uninstall\n  Config: {config_path} ({mcp.name} not found)")
                continue
...
def install_ida_plugin(*, uninstall: bool = False, quiet: bool = False):
    ...
    if uninstall:
        ...
        if realpath == IDA_PLUGIN_PY:
            if not quiet:
                print(f"Skipping IDA plugin installation\n  Path: {plugin_destination} (symbolic link)")
        else:
            shutil.copy(IDA_PLUGIN_PY, plugin_destination, follow_symlinks=False)
            if not quiet:
                print(f"Installed IDA Pro plugin (restart required)\n  Path: {plugin_destination}")
